...
class GraphConvolution(Module):
    ...
    def forward(self, H_v, H_e, adj_e, adj_v, T):
        if self.node_layer:
            ...
            mask1 = torch.eye(
                multiplier1.shape[0],
                device=multiplier1.device,
                dtype=multiplier1.dtype,
            )
            M1 = mask1 * torch.ones(
                multiplier1.shape[0],
                device=multiplier1.device,
            ...
            mask2 = torch.eye(
                multiplier2.shape[0],
                device=multiplier2.device,
                dtype=multiplier2.dtype,
            )
            M3 = mask2 * torch.ones(
                multiplier2.shape[0],
                device=multiplier2.device,
            ...
            dense_adjusted_A = adjusted_A.to_dense()
            # Perform normalization
            normalized_adjusted_A = dense_adjusted_A / dense_adjusted_A.max(0, keepdim=True)[0]
            # Convert back to sparse if needed
            sparse_normalized_adjusted_A = normalized_adjusted_A.to_sparse()
