...
class DygDataset(torch.utils.data.Dataset):
    def __init__(self, config, split_flag):
        ...
        self.dataset_name = '{}/{}.pkl'.format(config.dir_data, config.data_set)

        with open(self.dataset_name, 'rb') as file:
            data = pickle.load(file)
        self.input_nodes_feature, self.input_edges_feature, self.input_edges_pad, self.labels, self.Tmats, self.adjs, self.eadjs, self.mask_edge, self.ra = self.get_data(data, split_flag)
    def get_data(self, data, split_flag):
        ...
        return input_nodes_feature, input_edges_feature, input_edges_pad, labels, Tmats, adjs, eadjs, mask_edge, ra
